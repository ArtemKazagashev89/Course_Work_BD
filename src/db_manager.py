...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self) -> List[Tuple[str, int]]:
        ...
        with self.conn.cursor() as cursor:
            cursor.execute("SELECT name, COUNT(v.id) FROM companies c LEFT JOIN vacancies v ON c.id = v.company_id GROUP BY c.id")
    ...
    def get_all_vacancies(self) -> List[Tuple[str, str, int, int, str]]:
        ...
        with self.conn.cursor() as cursor:
            cursor.execute("SELECT v.title, c.name, v.salary_min, v.salary_max, v.url FROM vacancies v JOIN companies c ON v.company_id = c.id")
            return cursor.fetchall()
